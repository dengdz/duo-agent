...
| 模式 | 生效超时 | 默认值 |
|------|---------|--------|
| 普通（`enableReasoning(false)`） | `timeout` | 60 秒 |
| 推理（`enableReasoning(true)`） | `reasoningTimeout` | 5 分钟 |

超时触发时整轮失败，错误信息区分模式（"推理调用超时" / "LLM 调用超时"）。

HTTP 层还有一层兜底：Builder 自动按 `max(timeout, reasoningTimeout) + 1 分钟` 设置单请求整体超时——你配置多长的推理超时都不会被 HTTP 层提前掐断。

## 思考过程的可见性

推理模型输出分两部分，duo-agent 原生分离处理：

| 内容 | chat / stream | chatEvents |
|------|--------------|------------|
| 思考过程（`<think>`） | 不可见（过滤） | `ReasoningDelta` 增量实时推送 |
| 最终回答 | 正常输出 | `TextDelta` 增量推送 |

想在前端展示"模型正在思考…"的界面，用 `chatEvents()` 订阅 `ReasoningDelta`：

```java
agent.chatEvents("证明 √2 是无理数").subscribe(new Flow.Subscriber<>() {
...
**中途取消会怎样？**
`stream()`/`chatEvents()` 订阅者 `cancel()` 停止推送，但底层推理继续执行完毕（不中断 API 调用）。
