...
class DeepSeekAdapterTest {
    ...
    void testAgentLoop_whenRealApiCalled_thenCompletesTurn() throws Exception {
        ...
        var session = new dev.duo.core.session.Session(new dev.duo.model.session.SessionId("deepseek-test"));
        var agent = new dev.duo.core.agent.ReactLoopAgent(
                new dev.duo.model.session.SessionId("deepseek-agent"),
                new dev.duo.api.agent.AgentOptions("deepseek-official", MODEL, null, null),
                session,
                llm,
                new SystemPromptImpl("", false), new dev.duo.core.llm.ToolRegistryImpl()
        );
    ...
    void testToolRoundTrip_whenRealApiCalled_thenWritesTodos() throws Exception {
        ...
        var session = new dev.duo.core.session.Session(
                new dev.duo.model.session.SessionId("tool-test"));
        var agent = new dev.duo.core.agent.ReactLoopAgent(
                new dev.duo.model.session.SessionId("tool-agent"),
                new dev.duo.api.agent.AgentOptions("deepseek-official", MODEL, null, null),
                session,
                llm,
                sp,
        ...
        var userMsg = dev.duo.model.llm.MessageFactory.createUserMessage(
        ...
        var hasToolCall = session.events().stream()
                .anyMatch(e -> e instanceof dev.duo.model.session.SessionEventToolCall);
        var hasToolResult = session.events().stream()
                .anyMatch(e -> e instanceof dev.duo.model.session.SessionEventToolResult);
    ...
    void testParseChunk_whenToolCallDeltas_thenAssemblesToolCall() {
        ...
        parser.parseLine(chunk1, callback);
        parser.parseLine(chunk2, callback);
        parser.parseLine(chunk3, callback);
        parser.parseLine(chunk4, callback);
        parser.parseLine(chunk5, callback);
